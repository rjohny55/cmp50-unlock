...
| Автоматический Gen2 retrain | Не готов: обычный Retrain bit возвращает `RETRAIN_FAIL`; прямая Gen2-загрузка на ядре 138 приводит к Booter `0x8d`; двухэтапный v1 -> FLR -> v2 запускает GSP, но XVE shadow update не закрепляется и endpoint остаётся Gen1 |
