...
Проверка выполнена 26 августа 2026 года на четырёх картах:
...
| ReBAR live | PCI Region 1 = 16 ГБ; `nvidia-smi BAR1 Total` оставался 256 МиБ |
| ReBAR cold boot | Ещё не подтверждён на 20-ГБ host |
...
| Автоматический Gen2 retrain | Пока не готов: обычный Retrain bit возвращает `RETRAIN_FAIL` |
...
`--stage gen2` только собирает/ставит экспериментальный модуль и не выполняет
опасный цикл линий автоматически. Не включайте Gen2 на удалённой машине без
локального доступа и холодного восстановления.
