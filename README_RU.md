...
> Обновление 20 ГБ от 27 августа 2026: на эталонном X99 с ядром 137 полностью
> проверена автоматическая загрузка Gen2 без Link Disable. Fail-closed служба
> выполняет второй проход драйвера через FLR; четыре карты запускаются как
> Gen2 x4 + 3×x16, по 20480 МиБ VRAM и 16384 МиБ BAR1. Compute verifier
> проходит. Для ядра 138 по-прежнему проверены только стадии до ReBAR, поэтому
> установщик Gen2 на нём останавливается.
...
значение по умолчанию — `rt`, широко проверенная следующая стадия — `rebar`.
`gen2` проверен на эталонном X99 с ядром 137, остаётся зависимым от host и
защищён отдельным подтверждением в автоустановщике. Автоустановщик:
