...
нужно подтвердить на каждом новом host. `gen2` вместе с автоматическим вторым
проходом проверен только на эталонном X99 с ядром 6.8.0-137: 4 карты работают
как Gen2 x4 + 3×x16 с BAR1 16 ГиБ. На ядре 138 Gen2 не устанавливать.
...
CMP50_PATCH_STAGE=gen2 bash ./build.sh  # host-specific; установка только на 137
```
